...
public class recommenDAO {
	...
	public recommenDAO() {
	 ...
	 // '매칭신청' 버튼이 클릭된 해당 간병인의 정보만을 가져오기 위한 메서드
	 public List<RecommendInfoVO> chosenRecomInfo(String re_id, String g_id) {
		 List<RecommendInfoVO> list = new ArrayList<RecommendInfoVO>();
		 
		 try {
			 con = dataFactory.getConnection();
			 
			 String sql = "SELECT * FROM recommendations as r, caregiver as c WHERE r.re_id=? AND (c.g_id = ?)";
			 pstmt = con.prepareStatement(sql);
